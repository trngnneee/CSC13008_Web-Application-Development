    ...
    <>
      <Link href={`/product/${item.id_product}`}>
        <div className="p-5 bg-white shadow-2xl rounded-[10px] hover:scale-[1.02] transition-all duration-300 cursor-pointer">
          ...
          <div className="text-[20px] mt-[30px] border-b border-b-[black] pb-[10px] mb-[10px]">Current bid: $<span className="font-bold">{parseInt(item.price).toLocaleString("vi-VN")}</span></div>
          <div className="text-[10px] flex items-center gap-[5px]">
            <div className="w-4 h-4 rounded-full bg-amber-400"></div>
            <div>Kết thúc tại: <span className="font-bold">{dateFormat(item.end_date_time)}</span></div>
          </div>
          {isLogin ? (
