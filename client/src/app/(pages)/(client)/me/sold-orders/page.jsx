...
import { getMySoldOrders } from "@/lib/clientAPI/order";
...
export default function MySoldOrdersPage() {
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchOrders = async () => {
      const response = await getMySoldOrders();
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-3xl font-bold mb-6">Đơn hàng đã bán</h1>
            ...
            <div key={order.id_order} className="border rounded-lg p-4 bg-white shadow-sm">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600">Người mua: {order.winner_name}</p>
                  <p className="text-gray-600 text-sm">{order.winner_email}</p>
