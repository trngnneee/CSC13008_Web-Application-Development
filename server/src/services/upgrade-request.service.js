...
export const createUpgradeRequest = async (id_user) => {
  const [request] = await db('upgrade_request')
    .insert({ id_user, status: 'pending', created_at: new Date() })
    .returning('*');
  return request;
};

export const checkExistingRequest = async (id_user) => {
  return db('upgrade_request').where({ id_user }).where('status', 'pending').first();
};

export const getAllUpgradeRequests = async (filter = {}) => {
  const query = db('upgrade_request')
    .select(
      'upgrade_request.id_request', 'upgrade_request.id_user', 'upgrade_request.status',
      'upgrade_request.created_at', 'upgrade_request.reviewed_at', 'upgrade_request.reviewed_by',
  ...
  if (filter.status) query.where('upgrade_request.status', filter.status);
...
export const approveUpgradeRequest = async (id_request, id_admin) => {
  const request = await db('upgrade_request').where('id_request', id_request).first();
  if (!request) return null;

  await db.transaction(async (trx) => {
    await trx('upgrade_request').where('id_request', id_request).update({ status: 'approved', reviewed_at: new Date(), reviewed_by: id_admin });
    await trx('user').where('id_user', request.id_user).update({ role: 'seller' });
  });
...
export const rejectUpgradeRequest = async (id_request, id_admin) => {
  ...
  const [updatedRequest] = await db('upgrade_request')
    ...
    .update({ status: 'rejected', reviewed_at: new Date(), reviewed_by: id_admin })
    .returning('*');
