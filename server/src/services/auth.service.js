...
import { sendVerifyMail, sendOTPMail } from './mail.service.js';

export const handleRegister = async (userData, role) => {
  const { fullname, email, password } = userData;

  const existUser = await findUserByEmail(email, role);
  if (existUser) {
    return { success: false, message: `Email này đã được đăng ký với vai trò ${role}.` };
  }
  ...
  if (result) {
    ...
    await sendVerifyMail(email, verifyToken, role);
...
export const handleLogin = async (credentials, role) => {
  ...
  const token = jwt.sign(
    { id_user: existUser.id_user, email: existUser.email, role: existUser.role },
    process.env.JWT_SECRET,
    { expiresIn: rememberPassword === true ? '30d' : '1d' }
  );

  const tokenMaxAge = rememberPassword === true ? 30 * 24 * 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
  ...
  return {
    ...
    token,
    tokenMaxAge,
    user: { id_user: existUser.id_user, fullname: existUser.fullname, email: existUser.email },
  };
};
...
export const handleVerifyToken = async (token) => {
  ...
  try {
    ...
    if (!decoded) {
      return { success: false, message: 'Xác thực token thất bại!' };
    }

    const existUser = await findUserById(decoded.id_user);
    if (!existUser) {
      return { success: false, message: 'Tài khoản không tồn tại trong hệ thống!' };
    }

    const [watchList, userPoint, feedBackList] = await Promise.all([
      getUserWatchList(decoded.id_user),
      getUserPoint(decoded.id_user),
      getUserFeedbackList(decoded.id_user),
    ]);
    ...
    return { success: false, message: error.message || 'Xác thực token thất bại!' };
  }
};
...
export const handleForgotPassword = async (email, role) => {
  ...
  if (record) {
    ...
    return { success: true, message: 'Đã gửi OTP về email!' };
  }

  return { success: false, message: 'Lỗi gửi OTP về email!' };
};
...
export const handleVerifyEmail = async (token) => {
  ...
  return { success: true, message: 'Email xác nhận thành công!' };
};
