// server/helper/parse.helper.js
import { parse } from "csv-parse/sync";

// === KHAI BÁO DANH SÁCH FIELD CHUẨN ===
...
const parseDateFlexible = (v) => {
    ...
    if (m) {
        ...
        const iso = `${yyyy.padStart(4, "0")}-${mm.padStart(2, "0")}-${dd.padStart(2, "0")}T${String(
            HH
        ).padStart(2, "0")}:${String(MM).padStart(2, "0")}:${String(SS).padStart(2, "0")}Z`;
...
const convertField = (field, raw) => {
    const v = raw == null ? null : String(raw).trim();
    switch (field) {
        case "id_category": return v || null;      // nếu là số: đổi toNumber(v)
...
export function parseProductsCsv(buffer) {
